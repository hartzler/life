...
LifeClient.prototype.openDatabase = function() {
    ...
    try {
        ...
        if(!this.db.tableExists("objects")) {
            ...
            this.db.executeSimpleSQL("CREATE INDEX objects_by_type ON objects (type)");
        }
    ...
    this.st_insert_object = this.db.createStatement("INSERT OR REPLACE INTO objects (object_id,message_id,json,type) VALUES (:object_id,:message,:json,:type);");
    this.st_get_object = this.db.createStatement("SELECT objects.json FROM objects WHERE objects.object_id = :object");
    this.st_list_objects = this.db.createStatement("SELECT objects.object_id FROM objects WHERE objects.type = :type");
    this.st_list_objects_starting_at = this.db.createStatement("SELECT objects.object_id FROM objects WHERE objects.type = :type AND objects.object_id >= :start");
...
LifeClient.prototype.getOrInsertPerson = function(email) {
    this.logger.debug("getOrInsertPerson(" + email + ")");
                ...
                function(hits) {
                    ...
                    try {
                        for(var i = 0; i < hits.length; ++i) {
                            ...
                            if(!msg.objs[0].id)
                              msg.objs[0].id = uuid();
                            life.st_insert_object.params.object_id = msg.objs[0].id;
                            life.st_insert_object.params.message = message_id;
                            life.st_insert_object.params.type = msg.tag;
                            life.st_insert_object.params.json = JSON.stringify(msg.objs[0])
                            life.logger.debug("inserting object: " + life.st_insert_object.params.json);
...
LifeClient.prototype.get = function(id) {
    ...
    while(this.st_get_object.step()) {
        obj = JSON.parse(this.st_get_object.row.json)
        if(!obj) {
            this.logger.debug("getData: No json found for id: " + id);
            obj = {};
        }
    }
    return obj;
...
LifeClient.prototype.send = function(tag, to, subject, related, html, txt, obj, on_success, on_error) {
    this.logger.debug("LifeClient.send: tag=" + tag + " to=" + to.toSource() + " subject=" + subject + " obj=" + obj.toSource() );
    if(this.sender) {
    ...
    life.sender.connect(life.options["smtp_server"], life.options['email'], life.options['password'], 
        function() {
            life.logger.debug("sender connected");
